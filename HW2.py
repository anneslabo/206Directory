...
def string_manip(s):
    string1 = s.strip()
    #stripping string1 of white space before and after
    string2 = string1.upper()
    string3 = string2.replace(" ", "#")
    string4 = string3.replace("UMSI", '')
    lenstring = len(string4)
    if lenstring == 1:
        string5 = string4
    else:
         string5 = string4[::-1]
...
#names is a list of names
    ...
    for k, v in d.items():
      tmp.append( (k, v) )
    tmp = sorted(tmp, key = operator.itemgetter(0))
    tmp =  sorted(tmp, key = operator.itemgetter(1), reverse = True)
